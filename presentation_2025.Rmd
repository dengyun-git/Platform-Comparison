---
title: "Differences and Shared Characteristics of CSF Proteome Obtained from MS & Olink"
output:
  html_document:
    toc: true
    toc_float: 
...
# ALS_ClinicVarF <- rbind(ALS_ClinicVar1_F[intersect(colnames(ALS_ClinicVar1_F), colnames(ALS_ClinicVar2_F))], ALS_ClinicVar2_F[intersect(colnames(ALS_ClinicVar1_F), colnames(ALS_ClinicVar2_F))]) %>% select(CSF_OLINK_MANIFEST, GROUP) %>% filter(CSF_OLINK_MANIFEST != "")  %>% mutate(CSF_OLINK_MANIFEST = gsub("BX2", "BM2", CSF_OLINK_MANIFEST)) %>% mutate(CSF_OLINK_MANIFEST = gsub("BM1", "BX1", CSF_OLINK_MANIFEST)) 
